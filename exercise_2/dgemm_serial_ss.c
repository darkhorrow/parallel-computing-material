...
// dgemm_serial_ss.c
//
// Ejercicio 1 - Multiplicacion de matrices: C = B x A, versión no paralela
// Compilación: gcc -o dgemm_serial_ss dgemm_serial_ss.c -O2
//
#include <stdio.h>
#include <time.h>
...
void inicializa(){
  ...
  for(i = 0; i < Nmax; i++){ // inicializamos los vectores: a,b,c
        a[i] = (float) i; //lrand48(); // cuando se compruebe la funcionalidad correcta de la multiplicación, cambiar a aleatorio
        b[i] = a[i];
        c[i] = 0;
  }
}
